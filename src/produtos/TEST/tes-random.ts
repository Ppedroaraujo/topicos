...
import { Product } from '../src/produtos/products';
import { RandomValues } from '../src/produtos/random';
import { Erro } from '../src/produtos/erro';

let array = new RandomValues();
...
describe('RandomArray', () => {
    ...
    test('Deve retornar um erro se a quantidade solicitada for maior do que a quantidade total de produtos disponíveis', () => {
        ...
        expect(array.getRandomProduct(produtos, 5)).toBe(Erro.ARRAY_ERROR_LENGTH);
    });
    ...
    test('Deve retornar um erro se a quantidade for maior ou igual ao tamanho do array de Produtos', () => {
        ...
        expect(array.getRandomProduct(produtos, 3)).not.toBe(Erro.ARRAY_ERROR_LENGTH);
        expect(array.getRandomProduct(produtos, 4)).toBe(Erro.ARRAY_ERROR_LENGTH);
      });
    ...
    test('Deve receber um array de Produtos e uma quantidade igual ou menor que zero e retornar um erro', () => {
        ...
        expect(array.getRandomProduct(produtos, 0)).toBe(Erro.ARRAY_ERROR_QUANT);
        expect(array.getRandomProduct(produtos, -1)).toBe(Erro.ARRAY_ERROR_QUANT);
    });

    test('Deve retornar um erro se o array de produtos estiver vazio', () => {
        const produtos: Product[] = [];
        expect(array.getRandomProduct(produtos, 3)).toBe(Erro.ARRAY_ERROR_NULL);
    });
